...
def create_node(df, mode):
    if mode == 0: # onehot feature node, all 1 sample node
        ...
        feature_node[np.arange(n_col), feature_ind+1] = 1
        sample_node = np.zeros((n_row,n_col+1))
        sample_node[:,0] = 1
...
#%%
def create_edge_attr(df_X):
    nrow, ncol = df_X.shape
    edge_attr = []
    
    for i in range(nrow):
        for j in range(ncol):
            edge_attr.append([float(df_X.iloc[i,j])])
