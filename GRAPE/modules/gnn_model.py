"""
Reference:
[1] https://github.com/maxiaoba/GRAPE/blob/master/models/gnn_model.py
"""
#%%
import numpy as np
import torch
...
from modules.egcn import EGCNConv
from modules.egsage import EGraphSage
from modules.utils import get_activation
#%%
def get_gnn(data, config):
    #%%
    model_types = config['model_types'].split('_')
    
    if config['norm_embs'] is None:
        norm_embs = [True,]*len(model_types)
    else:
        norm_embs = list(
            map(
                bool, map(int, config['norm_embs'].split('_'))
            )
        )
    
    if config['post_hiddens'] is None:
        post_hiddens = [config['node_dim']]
    else:
        post_hiddens = list(
            map(int, config['post_hiddens'].split('_'))
        )
...
class GNNStack(nn.Module):
    def __init__(
        ...
        # post node update
        if concat_states:
            self.node_post_mlp = self.build_node_post_mlp(
                int(node_dim*len(model_types)), 
                int(node_dim*len(model_types)), 
                node_post_mlp_hiddens, 
                dropout, 
                activation
        ...
        self.edge_update_mlps = self.build_edge_update_mlps(
            node_dim, 
            edge_input_dim,
            edge_dim, 
            self.gnn_layer_num, 
    ...
    def build_edge_update_mlps(
        ...
        for _ in range(1, gnn_layer_num):
            edge_update_mlp = nn.Sequential(
    ...
    def forward(self, x, edge_attr, edge_index):
        ...
        for l, (conv_name,conv) in enumerate(zip(self.model_types, self.convs)):
            ...
            if self.concat_states:
                concat_x.append(x)
            edge_attr = self.update_edge_attr(
                x, edge_attr, edge_index, self.edge_update_mlps[l]
            )
