...
#%%
def kAnonymization(train_dataset, imputed):
    """
    ...
    return (
        evaluate_data(train),
        evaluate_data(imputed)
    )
#%%
def kMap(train_dataset, imputed):
    """
    ...
    for n_clusters in [2, 5, 10, 15]:
        ...
        clusters = cluster.predict(imputed)
    ...
    return np.min(values) / len(imputed) * 100
#%%
def DCR_metric(train_dataset, imputed, data_percent=15):
    """
    Reference:
    ...
    imputed = pd.DataFrame(scaler.transform(imputed), columns=imputed.columns)
    ...
    fake_sampled = imputed.sample(n=int(len(imputed)*(.01*data_percent)), random_state=42).to_numpy()
...
#%%
def attribute_disclosure(K, compromised, imputed, attr_compromised, dataset):
    dist = distance_matrix(
        compromised[attr_compromised].to_numpy(),
        imputed[attr_compromised].to_numpy(),
        p=2)
    ...
    for i in range(len(K_idx)):
        ...
        for j in range(len(dataset.categorical_features)):
            ...
            vote[j] = most_common(list(imputed.to_numpy()[K_idx[i], len(dataset.continuous_features) + j]))
        votes.append(vote)
        trues.append(true)
...
def AttributeDisclosure(train_dataset, imputed):
    """
    Reference:
        ...
        if attr_num > len(train_dataset.continuous_features):
            ...
            for K in [1, 10, 100]:
                acc = attribute_disclosure(
                    K, compromised, imputed_.copy(), attr_compromised, train_dataset)
                result.append(acc)
    AD = np.mean(result)
    return AD
#%%