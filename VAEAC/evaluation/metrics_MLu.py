...
#%%
def MLu_reg(train_dataset, test_dataset, imputed):
...
#%%
def MLu_reg_withmissing(test_dataset, imputed):
    test = test_dataset.raw_data.copy()
    imputed = imputed.copy()
    ...
    return syn_reg
#%%
def MLu_cls(train_dataset, test_dataset, imputed):
...
#%%
def MLu_cls_withmissing(test_dataset, imputed):
    ...
    mean = imputed_[continuous].mean()
    std = imputed_[continuous].std()
    test_[continuous] -= mean
    test_[continuous] /= std
    imputed_[continuous] -= mean
    imputed_[continuous] /= std
    
    covariates = [x for x in imputed_.columns if x not in [target]]
