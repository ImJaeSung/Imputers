...
def SMAPE(train_dataset, imputed):
    ...
    original = train_dataset.raw_data.values[:, :C].astype(np.float32)
    original_ = original[train_dataset.mask[:, :C] == 1]
    
    imputed = imputed.values[:, :C].astype(np.float32)
...
def WassersteinDistance(train_dataset, syndata, large=False):
    ...
    syndata_ = torch.tensor(syndata_, dtype=torch.float32)
