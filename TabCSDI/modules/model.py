...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, EncodedInfo, device):
        ...
        self.is_unconditional = config["is_unconditional"]
        ...
        self.diffmodel = diff_CSDI(config, input_dim)
    ...
    def forward(self, batch, mask, is_train=1):
        (
            observed_data,
            observed_mask,
            observed_tp,
...
class TabCSDI(CSDI_base):
    def __init__(self, config, EncodedInfo, device, target_dim=1):
        super(TabCSDI, self).__init__(target_dim, config, EncodedInfo, device)

    def process_data(self, batch, mask):
        # Insert K=1 axis. All mask now with shape (B, 1, L).
        observed_data = batch[:, np.newaxis, :]
        observed_data = torch.from_numpy(np.nan_to_num(observed_data))
        ...
        # observed_tp = torch.from_numpy(np.arange(self.EncodedInfo.num_features)) 
        observed_tp = np.arange(self.EncodedInfo.num_features) # should be equal to attributes number.
        observed_tp = np.tile(observed_tp, (observed_mask.shape[0], 1))
        observed_tp = torch.from_numpy(observed_tp)
